...
const SENTIMENT_ICONS = { POSITIVE: '\u{1F60A}', NEGATIVE: '\u{1F61F}', NEUTRAL: '\u{1F610}' };

const TYPE_META = {
  news: { icon: '\u{1F4F0}', label: 'News Analysis', desc: 'Media sentiment & risk alerts' },
  litigation: { icon: '\u2696\uFE0F', label: 'Litigation Search', desc: 'Court cases, NCLT & legal disputes' },
  promoter: { icon: '\u{1F464}', label: 'Promoter Profiling', desc: 'Background checks & web intelligence' },
  sector: { icon: '\u{1F4CA}', label: 'Sector Analysis', desc: 'Industry outlook & headwinds' },
  regulatory: { icon: '\u{1F3DB}\uFE0F', label: 'Regulatory Check', desc: 'Credit rating & compliance actions' },
};
    ...
    <div className="space-y-3 mt-2">
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        <div className="bg-red-50 border border-red-200 rounded p-2">
          <div className="font-medium text-red-800 mb-1">Headwinds</div>
        ...
        <button onClick={handleTrigger} disabled={loading}
          ...
          {loading ? (
            <span className="flex items-center justify-center gap-2">
              ...
              Running Intelligence Scan... (15-30 seconds)
            </span>
        ...
        <div className={`card border-l-4 ${triggerResponse.overall_risk === 'HIGH' ? 'border-l-red-500 bg-red-50' : triggerResponse.overall_risk === 'MEDIUM' ? 'border-l-yellow-500 bg-yellow-50' : 'border-l-green-500 bg-green-50'}`}>
          ...
          <div className="flex gap-4 mt-3">
            {Object.entries(triggerResponse.risk_summary || {}).map(([level, count]) => (
              <div key={level} className="text-sm"><RiskBadge level={level} /> x {count}</div>
            ))}
          </div>
        </div>
      )}
      ...
      {researchData && researchData.research_completed && (
        <div className="space-y-4">
          ...
          {Object.entries(researchData.results_by_type || {}).map(([type, items]) => {
            const meta = TYPE_META[type] || { icon: '\u{1F4CB}', label: type, desc: '' };
            return (
              <div key={type} className="card">
                <div className="flex justify-between items-center cursor-pointer" onClick={() => toggle(type)}>
