...
Uses REAL SpiceJet Annual Report + Web Research + Credit Scoring + CAM Generation
"""
import asyncio
import sys
...
async def test_spicejet_credit_analysis():
    """Complete credit analysis for SpiceJet using real data"""
    
    print_header("🚀 INTELLI-CREDIT: COMPLETE CREDIT ANALYSIS FOR SPICEJET")
    
    # Company details
    company_name = "SpiceJet Limited"
    mca_cin = "L62100DL2004PLC129768"  # SpiceJet's actual CIN
    sector = "Aviation"
    requested_limit_cr = 500.0  # 500 Crores loan request
    ...
    pdf_path = "AnnualReport_SpiceJet_202324.pdf"
    ...
    print(f"   Pending Litigations: {annual_report_data.get('pending_litigations', 'N/A')}")
    ...
    # Promoter Profiling (if directors known)
    print(f"\n👥 Promoter/Director Research...")
    promoter_profiler = PromoterProfiler(tavily_api_key=tavily_key)
    
    # SpiceJet's key promoters
    promoters = ["Ajay Singh", "SpiceJet Management"]
    promoter_findings = []
    ...
    revenue = annual_report_data.get('revenue_cr', 5000)
    total_debt = annual_report_data.get('total_debt_cr', 3000)
    net_worth = annual_report_data.get('net_worth_cr', 1000)
    ebitda = annual_report_data.get('ebitda_cr', 300)
    
    # Estimate DSCR and other ratios
    interest_estimate = total_debt * 0.10  # Assume 10% interest rate
    dscr = ebitda / interest_estimate if interest_estimate > 0 else 0
    debt_to_equity = total_debt / net_worth if net_worth > 0 else 999
    current_ratio = 1.2  # Would come from balance sheet
    ...
    scoring_input = {
        ...
        'financials': {
            ...
            'ebitda_cr': ebitda,
            'gst_vs_bank_variance': 2.5,  # Low variance (good)
            'circular_trading_score': 10   # Low risk
        },
        'research': {
        ...
        'sector': {
            'name': sector,
            'risk_level': 'HIGH'  # Aviation is high-risk sector
        },
        'due_diligence': {
            'notes': 'Aviation sector facing headwinds. Company has history of operational challenges.',
            'officer_assessment': 'CAUTIOUS'
        }
    }
    ...
    if final_decision == "APPROVE":
        ...
        print(f"   Monitoring: Regular quarterly reviews")
    elif final_decision == "CONDITIONAL_APPROVE":
        print(f"⚠️  RECOMMENDATION: CONDITIONAL APPROVAL")
        print(f"\n   Approved Limit: ₹{scoring_result['recommended_limit_cr']:.2f} Crores")
        print(f"   Conditions:")
        print(f"   • Additional collateral required")
        print(f"   • Personal guarantees from promoters")
        print(f"   • Monthly monitoring of cash flows")
        print(f"   • Restrictive covenants on debt levels")
    else:
        ...
        if news_data.get('negative_count', 0) > news_data.get('positive_count', 0):
            print(f"   • Negative news sentiment - Reputational concerns")
        if sector == "Aviation":
            print(f"   • High-risk sector - Aviation industry volatility")
        print(f"\n   Alternative Recommendations:")
        print(f"   • Reapply after improving DSCR to >1.5")
        print(f"   • Consider equity infusion to reduce leverage")
        print(f"   • Smaller working capital facility with collateral")
    
    # ==================== GENERATE SUMMARY ====================
    print_header("📝 EXECUTIVE SUMMARY")
    
    print(f"Based on comprehensive analysis of SpiceJet Limited:")
    print(f"\n1️⃣ DOCUMENT ANALYSIS")
    print(f"   • Annual Report FY 2023-24 parsed using Gemini Vision AI")
    print(f"   • Revenue: ₹{revenue:.2f} Crores")
    ...
    if final_decision == "CONDITIONAL_APPROVE":
        print(f"   ✓ Approve with enhanced monitoring")
        print(f"   ✓ Additional collateral/guarantees required")
        print(f"   ✓ Monthly cash flow reviews mandatory")
        print(f"   ✓ Maintain DSCR covenant of >1.25x")
    elif final_decision == "REJECT":
        print(f"   ✗ Not recommended for credit approval")
        print(f"   • Strengthen financial position before reapplying")
    else:
        print(f"   ✓ Approve as per standard terms")
...
if __name__ == "__main__":
    asyncio.run(test_spicejet_credit_analysis())
